...
<body>
  <div id="app">
    <div class="panel" id="topbar">
      <input id="roomInput" maxlength="120" placeholder="GAME ROOM" />
      ...
      <div id="modLayout">
        ...
        <div class="panel" id="modEditorPanel">
          ...
          <div class="editorField">
            ...
            <textarea id="modPrompt" placeholder="例: 盤面を少し暗くして、駒の見た目を落ち着いた感じにして..."></textarea>
          </div>
  ...
  <script type="module">
    ...
    (() => {
      ...
      const selectedModsEl = document.getElementById("selectedMods");
      const modSuggestEl = document.getElementById("modSuggest");
      ...
      const backFromMod = document.getElementById("backFromMod");
      ...
      const modPromptEl = document.getElementById("modPrompt");
      ...
      function currentTheme(){
        const patch = getRuntimePatch().theme || {};
        return {
          boardDark: patch.boardDark || BASE_THEME.boardDark,
          boardLight: patch.boardLight || BASE_THEME.boardLight,
          piece1: patch.piece1 || BASE_THEME.piece1,
          piece2: patch.piece2 || BASE_THEME.piece2
        };
      }
      ...
      function currentCooldownMs(){
        return Number(currentGamePatch().cooldownMs || 3000);
      }

      function currentHandCooldownMs(){
        return Number(currentGamePatch().handCooldownMs || 3000);
      }

      function currentCpuDelay(){
        const p = currentGamePatch();
        return {
          min: Number(p.cpuDelayMin ?? 1000),
          max: Number(p.cpuDelayMax ?? 1500),
          serious: Number(p.seriousCpuInterval ?? 100)
        };
      }
      ...
      function getRuntimePatch(){
        ...
        for(const mod of [...session.activeMods, ...session.roomMods]){
          patch = deepMerge(patch, mod.patch || {});
        }
        return patch;
      }
      ...
      function forceUpperRoomInput(){
        const start = roomInput.selectionStart;
        const end = roomInput.selectionEnd;
        const upper = roomInput.value.toUpperCase();
        if(roomInput.value !== upper){
          roomInput.value = upper;
          if(typeof start === "number" && typeof end === "number"){
            try{ roomInput.setSelectionRange(start, end); }catch{}
          }
        }
      }
      ...
      function deserializeMods(list){
        return (list || []).map(m => ({
          ...
          summary: m.summary || "",
          patch: deepMerge(getDefaultPatch(), m.patch || {})
        }));
      }
      ...
      function compileModVersion(mod, versionObj){
        return {
          ...
          patch: deepMerge(getDefaultPatch(), versionObj.patch || {})
        };
      }

      function buildSelectedModsFromRefs(spec){
        const refs = parseModRefs(spec);
        const out = [];
      ...
      function renderSelectedModsPills(){
        ...
        for(const mod of combinedAppliedMods()){
          ...
          pill.addEventListener("click", () => {
            ...
            const entries = parseModRefs(modSpec);
            const key = `${mod.name.toLowerCase()}@${mod.version}`;
            if(!entries.some(e => `${e.name.toLowerCase()}@${e.version || 0}` === key)){
              entries.push({ name: mod.name, version: mod.version });
            }
            roomInput.value = room ? `${room} MOD=${entries.map(e => e.version ? `${e.name} v${e.version}` : e.name).join(",")}` : `MOD=${entries.map(e => e.version ? `${e.name} v${e.version}` : e.name).join(",")}`;
      ...
      function renderModSuggestions(){
        const raw = normalizeRoomInputText(roomInput.value);
        ...
        for(const mod of list){
          ...
          chip.addEventListener("click", () => {
            ...
            if(!refs.length){
              roomInput.value = `${room} MOD=${mod.name}`;
            }else{
              roomInput.value = `${room} MOD=${next.map(r => r.version ? `${r.name} v${r.version}` : r.name).join(",")}`;
            }
            forceUpperRoomInput();
            renderModSuggestions();
            roomInput.focus();
      ...
      function renderModEditor(){
        ...
        if(!mod){
          versionPreview.textContent = "MODを選ぶと内容がここに出るよ.";
        ...
        if(cur){
          versionPreview.textContent = [
            ...
            `説明: ${cur.summary || "(なし)"}`,
            "",
            `テーマ変更: ${JSON.stringify(cur.patch?.theme || {}, null, 2)}`,
            `ゲーム設定: ${JSON.stringify(cur.patch?.game || {}, null, 2)}`,
            `表示設定: ${JSON.stringify(cur.patch?.ui || {}, null, 2)}`,
            `ルール設定: ${JSON.stringify(cur.patch?.rules || {}, null, 2)}`
          ].join("\n");
        }else{
          versionPreview.textContent = "このMODにはまだバージョンがないよ。AIで作るか保存してね。";
        }
      }

      function showModScreen(show){
        session.appMode = show ? "mod" : "game";
        modScreen.classList.toggle("show", show);
        if(show){
          renderModList();
          renderModEditor();
          setStatus("MOD制作画面だよ。");
      ...
      function createOrUpdateModFromEditor(){
        const name = normalizeModNameInput(modNameEl.value.trim());
      ...
      function pseudoMovesForType(side, type, x, y, pieces){
        ...
        if(type === "horse"){
          return dedupeMoves([...rayMoves([[-1,-1],[1,-1],[-1,1],[1,1]]), ...stepMoves([[0,-1],[0,1],[-1,0],[1,0]])]);
        }
        if(type === "dragon"){
          return dedupeMoves([...rayMoves([[0,-1],[0,1],[-1,0],[1,0]]), ...stepMoves([[-1,-1],[1,-1],[-1,1],[1,1]])]);
        }
        return [];
      }
      ...
      function updateMePill(){
        ...
        if(session.kind === "cpu_serious"){
          mePill.textContent = "SERIOUS CPU / あなた: 先手";
          return;
        }
      ...
      function getViewSide(){ return state.mySide === "white" ? "white" : "black"; }
      ...
      function drawPiece(piece, cell, now, selected){
        ...
        grad.addColorStop(0, t.piece1);
        grad.addColorStop(1, t.piece2);
        ctx.fillStyle = grad;
      ...
      function renderFrame(){
        ...
        for(const p of live.pieces){
          ...
          try{ drawPiece(p, { x:sp.x * cell, y:sp.y * cell, size:cell }, now, isSel); }
          catch(err){ console.error("drawPiece failed", err); }
        }
      ...
      function scoreAction(live, meta, actorSide){
        ...
        if(meta.nextMode === "promoted") score += 30;
        ...
        if(meta.pieceType === "pawn" || meta.pieceType === "lance"){
          ...
          score += (4 - Math.abs(meta.to.x - 4)) * 0.5;
          score += (4 - Math.abs(meta.to.y - 4)) * 0.5;
        }

        const key = aiActionKey(meta, actorSide);
        score += Math.max(-20, Math.min(20, aiAverage(key) * 2));
        score += Math.random() * 2;
        return score;
      }
      ...
      async function enterRoom(){
        ...
        try{
          ...
          const selectedMods = buildSelectedModsFromRefs(modSpec);
          session.activeMods = selectedMods;
          renderSelectedModsPills();
          applyRuntimeTheme();

          const ref = doc(db, "rooms", roomId);
          const snap = await getDoc(ref);
          const modSnapshot = serializeMods(selectedMods);
      ...
      enterBtn.addEventListener("click", openRoomOrModeFromInput);

      roomInput.addEventListener("input", () => {
        updateModUIFromInput();
      });
      ...
      shareBtn.addEventListener("click", async () => {
        const code = normalizeRoomInputText(roomInput.value || state.roomId || (session.kind === "cpu" ? "CPU" : session.kind === "cpu_serious" ? "SERIOUS CPU" : session.kind === "local_dual" ? "1 SMARTPHONE" : ""));
        if(!code){
      ...
      backFromMod.addEventListener("click", () => showModScreen(false));
      newModBtn.addEventListener("click", () => {
        ...
        session.currentVersion = null;
        saveCurrentModStore();
        modNameEl.value = "NEW MOD";
        modPromptEl.value = "";
        versionPreview.textContent = "AIに説明を書いてね。";
      });

      saveModBtn.addEventListener("click", createOrUpdateModFromEditor);
      aiGenerateBtn.addEventListener("click", createAIGeneratedModVersion);
      deleteModBtn.addEventListener("click", deleteCurrentMod);

      modNameEl.addEventListener("input", () => {
        modNameEl.value = normalizeModNameInput(modNameEl.value);
      });

      window.addEventListener("resize", resizeCanvas);

      function modStoreChanged(){
        saveModStore();
        renderModList();
        renderModEditor();
        renderSelectedModsPills();
      ...
      function idleTick(){
        scheduleRender();
        if(isCpuMode()) {
          if(state.roomData && !state.roomData.state?.winner) scheduleCpuThink(cpuThinkDelay());
        }
        setTimeout(idleTick, 250);
      }
